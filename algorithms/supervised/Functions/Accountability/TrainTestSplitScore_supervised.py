def get_train_test_split_score_supervised(model=None, training_dataset=not None, test_dataset=not None, factsheet=None, mappings=not None, target_column=None, outliers_data=None, thresholds=None, outlier_thresholds=None, outlier_percentage=None, high_cor=None, print_details=None):
    ...
    info, result, training_dataset, test_dataset = foo['info'], foo[
        'result'], foo['data']['training_dataset'], foo['data']['test_dataset']
    ...
    try:
        ...
        try:
            # shouldn'the try except catch the error it checks first for accoutnability then methodology key
            traintestsplit_mappings = foo['data']['mappings']["methodology"]["score_train_test_split"]["mappings"]["value"]
        except:
            # shouldn'the try except catch the error it checks first for accoutnability then methodology key
            traintestsplit_mappings = foo['data']['properties']["methodology"]["score_train_test_split"]["mappings"]["value"]
    ...
    try:
        training_data_ratio, test_data_ratio = train_test_split_metric(
